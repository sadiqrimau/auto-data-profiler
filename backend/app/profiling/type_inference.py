...
def infer_column_type(series: pd.Series) -> Tuple[str, float]:
    ...
    if bool_matches / total >= 0.95:
        return "boolean", float(round(bool_matches / total, 2))
    ...
    try:
        ...
        if integer_rate >= 0.95:
            return "integer", float(round(integer_rate, 2))
    except Exception:
        pass
    ...
    if numeric_rate >= 0.95:
        return "float", float(round(numeric_rate, 2))
    ...
    try:
        ...
        if date_rate >= 0.85:
            return "date", float(round(date_rate, 2))
    except Exception:
        pass
